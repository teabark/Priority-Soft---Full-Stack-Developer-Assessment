...
userSchema.methods.isCertifiedForLocation = function(locationId) {
  const certification = this.certifications.find(
    cert => cert.location.toString() === locationId.toString() && cert.isActive === true
  );
...
userSchema.methods.isAvailableAt = function(date, duration, timezone) {
  ...
  const recurringSlot = this.availability.find(slot => 
  ...
  // Check exceptions
  const exception = this.availability.flatMap(a => a.exceptions)
...
userSchema.methods.certifyForLocation = async function(locationId, certifiedBy) {
...
userSchema.methods.decertifyFromLocation = async function(locationId, reason, decertifiedBy) {
  const certification = this.certifications.find(
    cert => cert.location.toString() === locationId.toString() && cert.isActive
  );
...
userSchema.index({ 'certifications.location': 1, 'certifications.isActive': 1 });
