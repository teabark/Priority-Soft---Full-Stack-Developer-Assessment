...
const shiftHistorySchema = new mongoose.Schema({
  action: {
    type: String,
    enum: ['created', 'updated', 'assigned', 'unassigned', 'published', 'unpublished', 
           'cancelled', 'swap_requested', 'swap_approved', 'swap_rejected', 'completed'],
    required: true
  },
  performedBy: {
...
const complianceWarningSchema = new mongoose.Schema({
  type: {
    type: String,
    enum: ['overtime_weekly', 'overtime_daily', 'consecutive_days', 
           'gap_violation', 'skill_mismatch', 'certification_mismatch',
           'availability_violation', 'double_booking'],
    required: true
  },
  severity: {
    type: String,
...
shiftSchema.methods.unassignStaff = async function(staffId, unassignedBy, reason) {
  // Check if shift is editable
  if (!this.isEditable && this.status !== 'draft') {
    throw new Error('Cannot unassign staff from published shift within 48 hours');
  }
...
shiftSchema.methods.createSwapRequest = async function(requestData) {
  ...
  await requester.incrementPendingRequests(this.swapRequests[this.swapRequests.length - 1]._id);
  
  return this.swapRequests[this.swapRequests.length - 1];
};

// Method to approve swap request
shiftSchema.methods.approveSwapRequest = async function(requestId, approvedBy) {
  ...
  if (request.type === 'swap') {
    ...
    await this.unassignStaff(request.requestingStaff, approvedBy, 'Swap approved');
    await this.assignStaff(request.targetStaff, approvedBy);
  } else if (request.type === 'drop') {
    // For drop, just remove the staff
    await this.unassignStaff(request.requestingStaff, approvedBy, 'Drop approved');
...
shiftSchema.methods.rejectSwapRequest = async function(requestId, rejectedBy, reason) {
  const request = this.swapRequests.id(requestId);
...
shiftSchema.statics.getShiftsInRange = function(startDate, endDate, locationId = null) {
  const query = {
