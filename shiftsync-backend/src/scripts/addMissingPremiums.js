...
const addMissingPremiums = async () => {
  try {
    ...
    // Premium shifts to add - WEEKS 2-4
    const premiumShifts = [
      // ===== WEEK 2 (March 15-21) =====
      {
      ...
      {
        date: '2026-03-27', // Friday (same day, different staff)
        hour: 17,
        duration: 8,
        staff: sam,
        location: orlando,
    ...
    console.log(`\n📊 Adding ${premiumShifts.length} missing premium shifts...`);
    ...
    for (let i = 0; i < premiumShifts.length; i++) {
      ...
      await shift.save();
      console.log(`✅ Added: ${s.date} - ${s.staff.name} at ${s.location.name} (${s.skill})`);
    }

    console.log('\n🎉 All premium shifts added successfully!');

    // Verify final counts
    const totalPublished = await Shift.countDocuments({ status: 'published' });
    const totalPremium = await Shift.countDocuments({ isPremiumShift: true });
    
    console.log('\n📊 Final Counts:');
    console.log(`   Total published shifts: ${totalPublished}`);
